...
CONTINUITY_OFFSETS = (0, 90, 180, 270)
...
def sustain(db, patient, value="110", offsets=CONTINUITY_OFFSETS):
...
def qualify_warning(db, patient, value="110"):
    ...
    return ingest(db, patient, value, 300)


def test_hr_high_before_five_minutes_has_no_warning_alert(db_session, patient):
    sustain(db_session, patient)
    event = ingest(db_session, patient, "110", 299)
...
def test_hr_warning_qualifies_exactly_at_five_event_minutes(
    db_session,
    patient,
    value,
    condition,
    ...
    assert alert.confirmed_at == BASE_TIME + timedelta(minutes=5)
...
def test_normal_reading_ends_occurrence_and_next_abnormal_restarts(
    db_session,
    patient,
):
    ingest(db_session, patient, "110", 0)
    ingest(db_session, patient, "78", 60)
    tracker = tracker_for(db_session, patient)
    assert tracker.active is False

    restarted = ingest(db_session, patient, "110", 90)
...
def test_later_qualified_warning_evaluations_reuse_alert(db_session, patient):
    ...
    later = ingest(db_session, patient, "115", 330)
...
def test_warning_to_critical_reuses_and_escalates_same_alert(
    ...
    critical = ingest(db_session, patient, "151", 330)
    db_session.refresh(alert)
...
def test_critical_before_five_minutes_prevents_later_warning_duplicate(
    db_session,
    patient,
):
    ingest(db_session, patient, "110", 0)
    ingest(db_session, patient, "110", 90)
    critical = ingest(db_session, patient, "151", 120)
    ingest(db_session, patient, "110", 210)
    ingest(db_session, patient, "110", 300)
    later = ingest(db_session, patient, "110", 330)
    alerts = alerts_for(db_session, patient)
...
def test_normalization_allows_warning_in_new_occurrence(db_session, patient):
    ...
    new_qualifying = ingest(db_session, patient, "110", 660)
...
def test_concurrent_boundary_events_create_one_warning(
    ...
    concurrent_ingest(
        ...
        [
            ...
            {
                "numeric_value": "115",
                "recorded_at": BASE_TIME + timedelta(seconds=301),
            },
        ],
    )
...
def test_concurrent_different_hr_conditions_create_independent_warnings(
    integration_engine,
    db_session,
    patient,
):
    for offset in CONTINUITY_OFFSETS:
        ingest(db_session, patient, "110", offset)
        ingest(db_session, patient, "50", offset)
...
def test_warning_alert_failure_rolls_back_boundary_event(
    ...
    assert db_session.scalar(select(func.count(HealthEvent.event_id))) == prior_event_count
    assert tracker.last_seen_at == BASE_TIME + timedelta(seconds=270)
